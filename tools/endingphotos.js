...
(async function () {
  ...
  /* ★ 손으로 적어둔 설명(caption)과 보여줄 위치(focus)는 살립니다.
       예전에는 이 도구를 다시 돌릴 때마다 지워졌습니다 — 사진 한 장을 더 넣으려다
       "본관 드론샷" 같은 설명과 얼굴이 잘리지 않게 맞춰둔 값을 전부 잃었습니다.
       ⚠ 자리(01-, 02- …)로 찾습니다. 사진을 맨 뒤에 더하는 것은 안전하지만,
         가운데에 끼워 넣으면 그 뒤 사진들의 설명이 한 칸씩 밀립니다.
         그럴 때는 아래 표를 보고 손으로 옮기세요. */
  const kept = {};
  if (fs.existsSync(DATA_FILE)) {
    const old = fs.readFileSync(DATA_FILE, "utf8");
    const re = /\{\s*src:\s*"([^"]+)"[^}]*?\}/g;
    let m;
    while ((m = re.exec(old))) {
      const cap = /caption:\s*"((?:[^"\\]|\\.)*)"/.exec(m[0]);
      const foc = /focus:\s*"([^"]+)"/.exec(m[0]);
      if ((cap && cap[1]) || foc) {
        kept[path.basename(m[1])] = { caption: cap ? cap[1] : "", focus: foc ? foc[1] : "" };
      }
    }
  }
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    var hand = kept[outName] || {};
    out.push({
      src: "assets/img/ending/" + outName,
      year: item.year,
      caption: hand.caption || item.caption,
      focus: hand.focus || ""
    });
  ...
  const body = out.map(p =>
    ...
    (p.focus ? `, focus: ${JSON.stringify(p.focus)}` : "") + ` }`
  ).join(",\n");
